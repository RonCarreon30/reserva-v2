...
<div id="editRoomModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white py-4 px-6 rounded-md">
        ...
        <form id="editRoomForm" class="space-y-10">
            <div class="flex mb-4 gap-2">
                <div class="w-1/2">
                ...
                <div class="w-1/2">
                    ...
                    <input type="text" id="editRoomName" name="room_name" required class="w-full px-3 py-2 rounded-md border border-gray-300 focus:outline-none focus:border-blue-500">
                </div>
            </div>
            <div class="flex mb-4 gap-2">
                <div class="w-1/2">
                ...
                <div class="w-1/2">
                    ...
                    <select id="editRoomStatus" name="room_status" required class="w-full px-3 py-2 rounded-md border border-gray-300 focus:outline-none focus:border-blue-500">
                        <option value="Available">Available</option>
                        <option value="Unavailable">Unavailable</option>
                    </select>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <button type="submit" class="col-span-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Update Room</button>
            </div>
        </form>
    </div>
</div>
    ...
    <script>
        ...
        let currentRoomId;  // Declare a variable to store the current room ID

        // Edit Room
        function editRoom(id) {
            // Store the room ID for future use when saving changes
            currentRoomId = id;

            // Make an AJAX request to fetch the room details from the server using the room ID
        ...
        document.getElementById('editRoomForm').addEventListener('submit', function(event) {
            ...
            // Append the current room ID to the form data
        ...
        function confirmDelete() {
            fetch(`handlers/delete_room.php`, {
                ...
                body: JSON.stringify({ room_id: roomIdToDelete })
            })
            .then(response => response.json())
            .then(data => {
                closeDeleteModal(); // Close the modal after the response
                
                // Check the status and show a message based on it
                if (data.status === 'success') {
                    showToast(data);
                    setTimeout(() => {
                        location.reload(); // Reloads the current page
            ...
            .catch(error => {
                ...
                showToast('Error deleting the room.');
            });
        }
